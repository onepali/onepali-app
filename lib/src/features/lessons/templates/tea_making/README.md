...
**Entry point:** `KitchenPage` (`lib/src/features/tea_maker/pages/kitchen_page.dart`)  
**State machine:** `TutorialBloc` (`lib/src/features/tea_maker/bloc/`)  
...
5. On the last ingredient: show checkmark, wait, then play **tea ready** sound while showing bear + confetti at the same time.
...
| `bearTakingTeaTb` | `bear_taking_tea_tb` | Completion bear (tablet) |
| `bearTakingTeaMb` | `bear_taking_tea_mb` | Completion bear (mobile) |
...
initial
  ...
  → completed (bear + confetti + teaReadySound)
```
...
2. `await Future.delayed(Duration.zero)` so bear/confetti can paint.
3. Start `teaReadySound` with `unawaited(_playTeaReadySound)` — does **not** block the UI on audio length.

Bear images are **precached** in `KitchenPage.initState` via `MediaCacheManager` so the SVG is ready when completion fires.
...
- Full-screen bear (`bearTakingTeaMb` / `bearTakingTeaTb` by platform).
- Lottie confetti (`assets/lottie/confetti_1.json`).
- Tap bear area to pop navigator.
...
└── templates/tea_making/README.md  ← short template pointer
...
| Bear after sound | Handler blocked on `await _playAudioAwait(teaReadySound)` | Emit completed, `Duration.zero` yield, fire-and-forget audio; precache bear URLs |
...
- [ ] Completion: bear + confetti + `teaReadySound` at the same time.
