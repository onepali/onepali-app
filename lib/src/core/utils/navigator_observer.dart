...
class OrientationRouteObserver extends NavigatorObserver {
  ...
  bool _shouldBePortrait(Route<dynamic>? route) {
    ...
    if (name.isNotEmpty) {
      ...
      if (isPortrait) {
        ...
        logger.d('❌ Route "$name" is NOT in portraitRoutes list (defaulting to landscape)');
      }
    } else {
      logger.w('⚠️ Route has no name, cannot determine orientation from route name');
    }
  ...
  Future<void> _setOrientation(
    ...
    if (name == AppRoutes.parentPinScreen) {
      logger.d('🚫 Skipping orientation change for passcode screen (maintaining previous orientation)');
      return;
    }
    ...
    final isNavigatingFromDashboard = previousRoute != null && 
        (previousRoute.settings.name == AppRoutes.dashboardScreen);
    final isNavigatingToDrawer = name == AppRoutes.drawerRoutes || name == AppRoutes.tabDrawerRoutes;
    final isNavigatingFromDrawer = previousRoute != null && 
    ...
    if (context != null) {
      final currentOrientation = MediaQuery.of(context).orientation;
      isChangingOrientation = (shouldBePortrait && currentOrientation == Orientation.landscape) ||
                              (!shouldBePortrait && currentOrientation == Orientation.portrait);
    }
    
    // If not changing orientation, skip the orientation change
    if (!isChangingOrientation) {
      logger.d('🚫 Skipping orientation change: already in correct orientation');
      return;
    }
  ...
  void didPush(Route<dynamic> route, Route<dynamic>? previousRoute) {
    ...
    if (routeName == AppRoutes.drawerRoutes || routeName == AppRoutes.tabDrawerRoutes) {
      ...
      if (previousRoute != null) {
        ...
        if (!previousShouldBePortrait) {
          logger.d('🚫 Skipping orientation change: drawer and previous route (dashboard) are both landscape');
          super.didPush(route, previousRoute);
          return;
        }
      }
  ...
  void didPop(Route<dynamic> route, Route<dynamic>? previousRoute) {
    ...
    if (poppedRouteName == AppRoutes.parentPinScreen) {
      logger.d('🚫 Skipping orientation change when popping passcode screen (maintaining previous orientation)');
      super.didPop(route, previousRoute);
      return;
    }
    ...
    if (poppedRouteName == AppRoutes.drawerRoutes || poppedRouteName == AppRoutes.tabDrawerRoutes) {
      logger.d('🔄 Popping drawer route: $poppedRouteName, previous route: ${previousRoute?.settings.name ?? "unknown"}');
      if (previousRoute != null) {
        final previousRouteName = previousRoute.settings.name ?? '';
        final previousShouldBePortrait = _shouldBePortrait(previousRoute);
        logger.d('🔄 Previous route "$previousRouteName" should be ${previousShouldBePortrait ? "PORTRAIT" : "LANDSCAPE"}');
        ...
        if (!previousShouldBePortrait) {
          logger.d('🚫 Skipping orientation change: drawer and dashboard are both landscape');
          super.didPop(route, previousRoute);
          return;
        }
        ...
        _setOrientationForDrawerPop(previousRoute, context, previousShouldBePortrait);
        logger.d('✅ Orientation set to PORTRAIT after popping drawer (returning to Parent Zone)');
      }
      super.didPop(route, previousRoute);
      return;
    }
